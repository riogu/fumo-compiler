...
struct Codegen {
  ...
  public:
    ...
    void codegen(ASTNode* file_scope);
  ...
  private:
    ...
    constexpr llvm::Type* fumo_to_llvm_type(const Type& fumo_type) {
        switch (fumo_type.kind) {
            ...
            // case TypeKind::_enum:
            case Type::struct_: {
                auto type = llvm::StructType::getTypeByName(*llvm_context, get_id(fumo_type).name);
            ...
            default:
                INTERNAL_PANIC("couldn't get llvm::Type for '{}'", get_id(fumo_type).name);
        }
        return {};
    }
};
